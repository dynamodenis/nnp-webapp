...
import { loadSmes, updateSme } from "../../../redux/actions/smes";

function EditSmeForm(props) {
  const {updateSme, isLoading, smes} = props;
  const {id} = useParams();
  const form = useRef();
  ...
  // Get current vendor
  useEffect(() => {
    const sme = smes.filter(v => v?.id === id)
    setname(sme[0]?.name)
    setContact(sme[0]?.contact)
    setTel(sme[0]?.tel)
    setMail(sme[0]?.mail)
    setAddress(sme[0]?.address1)
    setTown(sme[0]?.town)
    setDesc(sme[0]?.descr)
    setSCounty(sme[0]?.scounty)
  },[id, smes])
...
export default connect(mapStateToProps,{updateSme,loadSmes})(React.memo(EditSmeForm));

...
import { addResearchCategory } from "../../../redux/actions/research_category";


function CreateResearchCategory(props) {
  const { user_roles, addResearchCategory } = props;
  const form = useRef();
  ...
  const createCourse = e => {
    ...
    addResearchCategory(body).then(res => {
      if (res === "success") {
        setname("");
        setType("");
      ...
      >
        <div className="md:pl-8 md:pr-8">
          <div className="flex flex-col sm:flex-row justify-between gap-2">
            <div>
              <div className="text-2xl font-medium">Add Research Category</div>
            </div>
          </div>
                  ...
                  <div className="pt-2">
                    ...
                    <div className="pt-2">
                      <TextValidator
                        ...
                        placeholder="Research category name"
                        type="text"
                        name="search"
                        value={name}
                        onChange={changename}
...
const mapStateToProps = state => ({
  isLoading: state.research_category.isAdding,
});

export default connect(mapStateToProps, {addResearchCategory })(React.memo(CreateResearchCategory));

...
import { updateProductCategory } from "../../../redux/actions/product_category";


function EditProductCategory(props) {
  const { isLoading, updateProductCategory,edit } = props;
  const form = useRef();
      ...
      >
        <div className="md:pl-8 md:pr-8">
          <div className="flex flex-col sm:flex-row justify-between gap-2">
            <div>
              <div className="text-2xl font-medium">Edit Product Category</div>
            </div>
          </div>
...
const mapStateToProps = state => ({
  isLoading: state.product_category.isUpdating,
});

export default connect(mapStateToProps, { updateProductCategory })(React.memo(EditProductCategory));
